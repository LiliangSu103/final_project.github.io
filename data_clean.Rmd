...
crash_df = read_csv("./data/Motor_Vehicle_Collisions_-_Crashes_20251031.csv", na = c("Unspecified", "Does Not Apply", "Unknown", "NA", ".", "")) |> 
  ...
  select(-location, -borough, -crash_time,- crash_date_full, -starts_with("contributing_factor"), -starts_with("vehicle_type"), -starts_with("number_of")) |> 
...
Use geom_bin2d to create a 2D histogram/heatmap.
...
manhattan_heatmap =
  ...
  ggplot(aes(x = longitude, y = latitude, weight = num_casualty)) +
  
  geom_density_2d_filled(alpha = 0.8) +
  scale_fill_viridis_d(option = "C", name = "Collision Casualty Density") +
  coord_cartesian(xlim = c(LON_MIN, LON_MAX), ylim = c(LAT_MIN, LAT_MAX)) +
    labs(
    title = "Heatmap of Collision Casualties in Manhattan (2021-2025)",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_bw() +
